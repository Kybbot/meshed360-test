...
import { useGetAccounts } from "@/entities/selects";
...
export const FormTableAccountSelect = forwardRef<Ref, Props>(
	({ id, type, error, value, disabled, placeholder = "Select", onValueChange }, forwardedRef) => {
		...
		const {
			...
			isLoading,
			error: dataError,
		} = useGetAccounts({ organisationId: orgId, searchValue: searchValue, type });

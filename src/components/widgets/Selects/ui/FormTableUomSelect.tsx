...
import { useGetUnitsOfMeasure } from "@/entities/selects";
...
type Props = {
	...
	onValueChange: (value: UnitOfMeasureType | undefined) => void;
};

type Ref = HTMLDivElement;

export const FormTableUomSelect = forwardRef<Ref, Props>(
	({ id, error, value, disabled, placeholder = "Select", onValueChange }, forwardedRef) => {
		...
		const {
			...
			isLoading,
			error: dataError,
		} = useGetUnitsOfMeasure({ organisationId: orgId, searchValue: searchValue });
		...
		const selectOption = (option: UnitOfMeasureType) => {
			if (option !== value) onValueChange(option);
		};

		const isOptionSelected = (option: UnitOfMeasureType) => {
			return option.id === value?.id;
		};

		const filteredValues = useMemo(() => {
			if (data) {
				return data.data.unitsOfMeasure.filter(

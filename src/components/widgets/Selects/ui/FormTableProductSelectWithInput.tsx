...
export const FormTableProductSelectWithInput = forwardRef<Ref, Props>(
	({ id, type, error, value, disabled, placeholder = "Select", onValueChange }, forwardedRef) => {
		...
		const hanldeInputChange = (event: ChangeEvent<HTMLInputElement>) => {
			...
			const newObj = inputValue ? ({ id: "", name: inputValue } as ProductType) : undefined;
		...
		const filteredValues = useMemo(() => {
			if (data) {
				return data.data.allProducts.filter(
			...
			<PopoverPrimitive.Root>
				<PopoverPrimitive.Trigger asChild>
					<div
						className={`
							...
							customTableSelect__trigger--input
					...
					>
						<input
							...
							className={`tableInput__field tableInput__field--noBorder ${hasError ? "tableInput__field--error" : ""}`}
						/>
				...
				<PopoverPrimitive.Portal container={document.body.querySelector("#portal2") as HTMLDivElement}>
					<PopoverPrimitive.Content
						align="start"
						ref={forwardedRef}
						className="tableSelect2__content"

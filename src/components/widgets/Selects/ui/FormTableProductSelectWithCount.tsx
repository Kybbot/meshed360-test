...
import { ProductForSelect } from "@/pages/Orders/EditOrder/hooks/useProductsSelect.ts";
...
type Props = {
	...
	value?: ProductType;
	placeholder?: string;
	onValueChange: (value?: ProductType) => void;
...
export const FormTableProductSelectWithCount = forwardRef<Ref, Props>(
	(
		{ id, error, value, disabled, placeholder = "Select", onValueChange, products, isLoading },
		forwardedRef,
	) => {
		...
		const selectedOption = useMemo(() => {
			return products.find(({ id }) => value?.id === id);
		}, [value, products]);

		const isOptionSelected = (option: ProductType) => {
		...
		return (
			<PopoverPrimitive.Root>
				<PopoverPrimitive.Trigger asChild>
					...
					>
						<span>
							{selectedOption ? `${selectedOption.name} - max (${selectedOption.count})` : <>&#8203;</>}
						</span>
						<span
							className={`customTableSelect__placeholder ${selectedOption?.name ? "customTableSelect__placeholder--active" : ""}`}
						>
							{placeholder}
						</span>

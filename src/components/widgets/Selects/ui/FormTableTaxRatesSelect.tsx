...
import { useGetTaxRates } from "@/entities/selects";
...
type Props = {
	...
	value?: TaxRateType;
	type?: "sales" | "purchase";
	onValueChange: (value: TaxRateType | undefined) => void;
};

type Ref = HTMLDivElement;

export const FormTableTaxRatesSelect = forwardRef<Ref, Props>(
	({ id, type, error, value, disabled, placeholder = "Select", onValueChange }, forwardedRef) => {
		...
		const {
			...
			isLoading,
			error: dataError,
		} = useGetTaxRates({ organisationId: orgId, searchValue: searchValue, type });
		...
		const selectOption = (option: TaxRateType) => {
			if (option !== value) onValueChange(option);
		};

		const isOptionSelected = (option: TaxRateType) => {

...
import { GetAssemblyPickResponseType, UndoAssemblyPickStatusResponseType } from "@/@types/assembly/pick";
import { GetAssemblyByIdResponseType } from "@/@types/assembly/assembly";

export const useUndoAssemblyPick = () => {
	const queryClient = useQueryClient();

	return useMutation<
		ApiResult<UndoAssemblyPickStatusResponseType>,
		AxiosError<ApiError>,
		{
			body: { id: string };
		}
	>({
		mutationFn: async ({ body }) => {
			const { data } = await axiosInstance.post<ApiResult<UndoAssemblyPickStatusResponseType>>(
				`/api/assemblies/pick/undo`,
				JSON.stringify(body),
			);
		...
		onSuccess: async (newData, variables) => {
			toast.success("Assembly Pick was successfully undo");

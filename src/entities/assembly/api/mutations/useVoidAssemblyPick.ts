...
import { GetAssemblyPickResponseType, VoidAssemblyPickStatusResponseType } from "@/@types/assembly/pick";
import { GetAssemblyByIdResponseType } from "@/@types/assembly/assembly";

export const useVoidAssemblyPick = () => {
	const queryClient = useQueryClient();

	return useMutation<
		ApiResult<VoidAssemblyPickStatusResponseType>,
		AxiosError<ApiError>,
		{
			body: { id: string };
		}
	>({
		mutationFn: async ({ body }) => {
			const { data } = await axiosInstance.post<ApiResult<VoidAssemblyPickStatusResponseType>>(
				`/api/assemblies/pick/void`,
				JSON.stringify(body),
			);
		...
		onSuccess: async (newData, variables) => {
			toast.success("Assembly Pick was successfully voided");

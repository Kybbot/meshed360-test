...
export const useAuthoriseInvoice = () => {
	...
	>({
		...
		mutationFn: async ({ organisationId, orderId, invoiceId, body }) => {
			...
			const { data } = await axiosInstance.post<
				ApiResult<{ salesOrderInvoice: InvoiceType; salesOrder: ExtendedSalesOrder }>
			>(
				`/api/sales-orders/${organisationId}/orders/${orderId}/invoices/${invoiceId}/authorize`,
				JSON.stringify(body),
			);
		...
		onSuccess: async (data, { organisationId, orderId }) => {
			...
			toast.success("Invoice was successfully authorized");
		},
		onError(error) {
			showError(error);
		},

...
export const useSaveRestock = () => {
	...
	>({
		mutationKey: ["save-restock"],
		mutationFn: async ({ organisationId, orderId, body }) => {
			const { data } = await axiosInstance.post<
				ApiResult<{ salesOrderRestock: RestockType; salesOrder: ExtendedSalesOrder }>
			>(`/api/sales-orders/${organisationId}/orders/${orderId}/restock/save`, JSON.stringify(body));
		...
		onSuccess: async (_, { organisationId, orderId }) => {
			...
			toast.success("Restock was successfully saved");
		},
		onError(error) {
			showError(error);
		},

...
export const useVoidRestock = () => {
	...
	>({
		mutationKey: ["void-restock"],
		mutationFn: async ({ organisationId, orderId }) => {
			const { data } = await axiosInstance.post<
				ApiResult<{ salesOrderRestock: RestockType; salesOrder: ExtendedSalesOrder }>
			>(`/api/sales-orders/${organisationId}/orders/${orderId}/restock/void`);
		...
		onSuccess: async (_, { organisationId, orderId }) => {
			...
			toast.success("Restock was successfully voided");
		},
		onError(error) {
			showError(error);
		},

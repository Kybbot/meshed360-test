...
export const useVoidPacking = () => {
	...
	>({
		mutationKey: ["void-packing"],
		mutationFn: async ({ organisationId, orderId, fulfillmentId }) => {
			const { data } = await axiosInstance.post<
				ApiResult<{ fulfillment: FulfilmentType; salesOrder: ExtendedSalesOrder }>
			>(`/api/sales-orders/${organisationId}/orders/${orderId}/fulfillments/${fulfillmentId}/packing/void`);
		...
		onSuccess: async (data, { organisationId, orderId }) => {
			...
			toast.success("Packing was successfully voided");
		},
		onError(error) {
			showError(error);
		},

...
export const useAuthoriseShipping = () => {
	...
	return useMutation<
		...
		AxiosError<ApiError>,
		{
			...
			body: UpdateFulfillmentShippingType;
		}
	>({
		mutationKey: ["authorise-shipping"],
		mutationFn: async ({ organisationId, orderId, fulfillmentId, body }) => {
			await axiosInstance.post(
				`/api/sales-orders/${organisationId}/orders/${orderId}/fulfillments/${fulfillmentId}/shipping/save`,
				JSON.stringify(body),
			);
			...
			const { data } = await axiosInstance.post<
				ApiResult<{ fulfillment: FulfilmentType; salesOrder: ExtendedSalesOrder }>
			>(
				`/api/sales-orders/${organisationId}/orders/${orderId}/fulfillments/${fulfillmentId}/shipping/authorize`,
				JSON.stringify(body),
			);
		...
		onSuccess: async (data, { organisationId, orderId }) => {
			...
			toast.success("Shipping was successfully authorized");
		},
		onError(error) {
			showError(error);
		},

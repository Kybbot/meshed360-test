...
export const useVoidPicking = () => {
	...
	>({
		mutationKey: ["void-picking"],
		mutationFn: async ({ organisationId, orderId, fulfillmentId }) => {
			const { data } = await axiosInstance.post<
				ApiResult<{ fulfillment: FulfilmentType; salesOrder: ExtendedSalesOrder }>
			>(`/api/sales-orders/${organisationId}/orders/${orderId}/fulfillments/${fulfillmentId}/picking/void`);
			return data;
		},
		onSuccess: async (data, { organisationId, orderId }) => {
			...
			toast.success("Picking was successfully voided");
		},
		onError(error) {
			showError(error);
		},

...
import {
	...
	UpdateFulfillmentPickingType,
} from "@/@types/salesOrders/api.ts";

export const useAuthorisePicking = () => {
	...
	return useMutation<
		...
		AxiosError<ApiError>,
		{
			...
			body: UpdateFulfillmentPickingType;
		}
	>({
		mutationKey: ["authorise-picking"],
		mutationFn: async ({ organisationId, orderId, fulfillmentId, body }) => {
			await axiosInstance.post(
				`/api/sales-orders/${organisationId}/orders/${orderId}/fulfillments/${fulfillmentId}/picking/save`,
				JSON.stringify(body),
			);
			...
			const { data } = await axiosInstance.post<
				ApiResult<{ fulfillment: FulfilmentType; salesOrder: ExtendedSalesOrder }>
			>(
				`/api/sales-orders/${organisationId}/orders/${orderId}/fulfillments/${fulfillmentId}/picking/authorize`,
				JSON.stringify(body),
			);
		...
		onSuccess: async (data, { organisationId, orderId }) => {
			...
			toast.success("Picking was successfully authorized");
		},
		onError(error) {
			showError(error);
		},

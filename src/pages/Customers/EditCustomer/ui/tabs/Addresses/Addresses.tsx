...
import { useGetAllCustomerAddresses } from "../../../api/queries/useGetAllCustomerAddresses";
...
import { CustomerAddressesType, NormalizedAddressesData } from "@/@types/customers";
...
export const Addresses: FC = () => {
	...
	const { data, error, isLoading, isError, isSuccess } = useGetAllCustomerAddresses({
		pageNumber: page,
		pageSize: limit,
		customerId,
	});

	const { mutate: updateAddress, isPending: isUpdateAddressLoading } = useUpdateCustomerAddress(true);

	const handlePageAndLimit = (page: string, limit: string) => {
		setPageAndLimit({ page, limit });
	};

	const handleChangeCheckbox = (value: CustomerAddressesType[], itemIndex: number, checked: boolean) => {
		if (customerId) {
			...
			updateAddress({ customerId, body: { addresses: updatedArr } });
		}
	};
			...
			{isLoading ? (
				...
				<>
					<div className="commonPage__actions commonPage__actions--complex">
						<div />
						<Button type="button" onClick={() => handleOnNew(data.data)} disabled={!customerId}>

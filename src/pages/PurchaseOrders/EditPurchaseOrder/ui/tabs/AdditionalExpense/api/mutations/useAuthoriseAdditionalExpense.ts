...
import {
	...
	AuthoriseAdditionalExpenseStatusResponseType,
} from "@/@types/purchaseOrder/additionalExpense";
import { ApiError, ApiResult } from "@/@types/api";
import { GetPurchaseOrderByIdResponseType } from "@/@types/purchaseOrder/order";

export const useAuthoriseAdditionalExpense = () => {
	const queryClient = useQueryClient();

	return useMutation<
		ApiResult<AuthoriseAdditionalExpenseStatusResponseType>,
		AxiosError<ApiError>,
		{
	...
	>({
		mutationFn: async ({ body }) => {
			const { data } = await axiosInstance.post<ApiResult<AuthoriseAdditionalExpenseStatusResponseType>>(
				`/api/purchase-orders/expenses/authorise`,
				JSON.stringify(body),
			);
		...
		onSuccess: async (newData, variables) => {
			toast.success("Additional Expenses was successfully authorised");

...
export const BillsSelect = forwardRef<Ref, Props>(
	({ id, error, billIds, disabled, receivings, customValues, onValueChange }, forwardedRef) => {
		...
		const bills = useMemo(() => {
			return getBillsForSelect(customValues, receivings);
		}, [customValues, receivings]);

		const filteredValues = useMemo(() => {
			if (bills.billsResult) {
				return bills.billsResult.filter(
					(item) =>
						item.status === "AUTHORIZED" &&
						item.billNumber.toLowerCase() !== null &&

...
import { EmptyRestockLine, RestockFormValues } from "@/@types/salesOrders/local.ts";
...
type Props = {
	...
	control: Control<RestockFormValues, unknown>;
	update: UseFieldArrayUpdate<RestockFormValues, "lines">;
	fields: FieldArrayWithId<RestockFormValues, "lines", "id">[];
	productsSelect: { products: ProductForSelect[]; isLoading: boolean };
};

export const RestockLinesRow: FC<Props> = ({ index, fields, control, remove, update, productsSelect }) => {
	const {
		trigger,
		register,
		setValue,
	...
	return (
		<TFTr>
			...
			<TFTd>
				<TableInputRhf<RestockFormValues>
					min={0}
					step={0.01}
					type="number"
					label="Quantity"

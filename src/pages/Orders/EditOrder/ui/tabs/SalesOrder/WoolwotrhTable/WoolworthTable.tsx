...
import { OrderLines } from "@/pages/Orders/EditOrder/ui/tabs/Quote/WoolwotrhTable/OrderLines.tsx";
import { OrderServices } from "@/pages/Orders/EditOrder/ui/tabs/Quote/WoolwotrhTable/OrderServices.tsx";
...
import { getNormalizedWoolworthQuoteData } from "@/pages/Orders/EditOrder/ui/tabs/Quote/utils/getNormalizedWoolworthData.ts";
...
export const WoolworthTable: FC<Props> = ({ order, quote }) => {
	...
	} = useForm<WoolworthOrderFormValues>({
		defaultValues: {
		...
		if (order) {
			reset({
				lines: order?.orderLines.length
					? order.orderLines.map((line): WoolworthsOrderLineType => {
							return {
	...
	const onSubmit = async (
		formData: WoolworthOrderFormValues,
		action: "save" | "email" | "authorise" | "close",
	) => {
		const payload = disabled ? undefined : getNormalizedWoolworthQuoteData(formData);
			...
			{isLoading ? (
				...
				<>
					...
					<CommonPageMain>
						...
						<CommonPageFooter>
							<TextareaRhf<WoolworthOrderFormValues>
								name="memo"
								id="memoId"
								label="Quote Memo"
								register={register}

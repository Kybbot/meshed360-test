...
import { OrderLines } from "@/pages/Orders/EditOrder/ui/tabs/Quote/MeatTable/OrderLines.tsx";
import { OrderServices } from "@/pages/Orders/EditOrder/ui/tabs/Quote/MeatTable/OrderServices.tsx";
...
export const MeatTable: FC<Props> = ({ order, quote }) => {
		...
		if (order) {
			reset({
				lines: order?.orderLines.length
					? order?.orderLines.map((line): MeatOrderLineType => {
							return {
	...
	const onSubmit = async (
		formData: MeatOrderFormValues,
		action: "save" | "email" | "authorise" | "close",
	) => {
		const payload = disabled ? undefined : getNormalizedMeatQuoteData(formData);

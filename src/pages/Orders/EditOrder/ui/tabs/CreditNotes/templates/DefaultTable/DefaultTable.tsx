...
import { OrderLines } from "@/pages/Orders/EditOrder/ui/tabs/Invoicing/templates/DefaultTable/OrderLines.tsx";
import { OrderServices } from "@/pages/Orders/EditOrder/ui/tabs/Invoicing/templates/DefaultTable/OrderServices.tsx";
...
import { getNormalizedDefaultInvoiceData } from "@/pages/Orders/EditOrder/ui/tabs/Invoicing/utils/getNormalizedDefaultInvoiceData.ts";
...
export const DefaultTable: FC<Props> = ({
	creditNote,
	orderInfo,
	isListShown,
	setIsListShown,
	...
	const form = useForm<DefaultInvoicingFormValues>({
		defaultValues: {
		...
		if (creditNote) {
			form.reset({
				lines: creditNote?.lines.length
					? creditNote?.lines.map((line): DefaultOrderLineType => {
							return {
					...
					: [EmptyDefaultOrderLine],
				serviceLines: creditNote?.lines.length
					? creditNote?.serviceLines.map((line): OrderServiceLineType => {
							return {
								product: line.product as unknown as ProductType,
								comment: line.comment || "",
								quantity: line.quantity,
	...
	const onSubmit = async (formData: DefaultInvoicingFormValues, action: "save" | "authorise" | "void") => {
		const payload = getNormalizedDefaultInvoiceData(formData, priceListData!.data.priceList.currencyId);

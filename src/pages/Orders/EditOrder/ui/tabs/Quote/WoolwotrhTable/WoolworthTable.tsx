...
export const WoolworthTable: FC<Props> = ({ quote, orderInfo }) => {
		...
		if (quote) {
			reset({
				lines: quote?.lines.length
					? quote.lines.map((line): WoolworthsOrderLineType => {
							return {
	...
	const onSubmit = async (formData: WoolworthOrderFormValues, action: "save" | "authorise" | "void") => {
		const payload = getNormalizedWoolworthQuoteData(formData);

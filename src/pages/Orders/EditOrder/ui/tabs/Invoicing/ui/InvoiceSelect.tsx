...
import { InvoiceType } from "@/@types/salesOrders/api.ts";

type Props = {
	invoices: InvoiceType[];
	disabled?: boolean;
	onValueChange: (value: InvoiceType) => void;
};

type Ref = HTMLDivElement;

export const InvoiceSelect = forwardRef<Ref, Props>(({ invoices, disabled, onValueChange }, forwardedRef) => {
	...
	const filteredValues = useMemo(() => {
		if (invoices) {
			return invoices.filter((item) => item.status === "AUTHORIZED");
		}
	...
	return (
		<div className="customSelect__wrapper fulfilmentSelect" ref={forwardedRef} style={{ minWidth: "204px" }}>
			<div className="customSelect__container" ref={containerRef}>
				...
				>
					<span>&#8203;</span>
					<span className={`customSelect__placeholder`}>Copy From Invoice</span>
					<div
						className={`
								customSelect__icon

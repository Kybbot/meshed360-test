...
import { getDateFromDayPickerDate, getDueDate } from "@/utils/date";
		...
		if (invoice) {
			form.reset({
				lines: invoice?.lines.length
					? invoice?.lines.map((line): DefaultOrderLineType => {
							return {
					...
					: [EmptyDefaultOrderLine],
				serviceLines: invoice?.lines.length
					? invoice?.serviceLines.map((line): OrderServiceLineType => {
							return {
								product: line.product as unknown as ProductType,
								comment: line.comment || "",
								quantity: line.quantity,

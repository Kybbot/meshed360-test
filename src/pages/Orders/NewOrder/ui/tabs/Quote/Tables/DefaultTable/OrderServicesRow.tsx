...
export const OrderServicesRow: FC<Props> = ({
	...
	const onProductChange = (value: ProductType) => {
		...
		if (value.salesTax) {
			...
			setValue(`serviceLines.${index}.taxType`, orderTax);
		}
		...
		if (value.serviceAccountMappings.revenueAccount) {
			...
			if (defaultAccount) setValue(`serviceLines.${index}.account`, defaultAccount);
		} else {
			setValue(`serviceLines.${index}.account`, orderAccount);
		}
		...
		if (customerId) {
			...
			if (discount != null) {
				setValue(`serviceLines.${index}.discount`, discount);
			}
		}
	};

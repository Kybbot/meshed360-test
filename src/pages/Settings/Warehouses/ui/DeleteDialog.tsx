...
import { useDeleteWarehouse } from "../api/mutations/useDeleteWarehouse";
...
export const DeleteDialog: FC<DeleteDialogProps> = ({
	...
	const { mutate, error, isError, isPending, isSuccess } = useDeleteWarehouse();
	...
	const handleOnSubmit = () => {
		if (currentItem.id) {
			mutate({ warehousesId: currentItem.id });
		}
	};
		...
		<DialogRoot open={dialogState} onOpenChange={setDialogState}>
			<DialogContent size="small" onInteractOutside={handleCloseModal} onEscapeKeyDown={handleCloseModal}>
				<div className="infoDialog">
					...
					<DialogTitle asChild>
						<h3 className="infoDialog__title">Delete Warehouse</h3>
					</DialogTitle>

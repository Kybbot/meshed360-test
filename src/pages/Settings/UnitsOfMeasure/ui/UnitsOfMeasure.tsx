...
import { useGetAllUnitsOfMeasure } from "../api/queries/useGetAllUnitsOfMeasure";
...
const UnitsOfMeasure: FC = () => {
	...
	const { data, error, isLoading, isError, isSuccess } = useGetAllUnitsOfMeasure({
			...
			<div className="main__container">
				<CommonPageWrapper>
					{isLoading ? (
						...
						<>
							<CommonPageHeader>
								<CommonPageTitle>Units of Measure</CommonPageTitle>
								<CommonPageActions>
									<Search
										id="searchId"
										name="search"
								...
								<div className="customTable">
									<div className="customTable__wrapper">
										<Table>
											...
											<TableTbody>
												{!data.data.unitsOfMeasure.length ? (
													<TableEmpty colSpan={2} />
												) : (
													<>
														{data.data.unitsOfMeasure.map((item) => (
															<TableTr key={item.id}>

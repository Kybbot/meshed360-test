...
import { useUpdateUnitOfMeasure } from "../api/mutations/useUpdateUnitOfMeasure";
...
export const EditDialog: FC<EditDialogProps> = ({
	...
	const { isPending, isSuccess, isError, error, mutate } = useUpdateUnitOfMeasure();
	...
	const onSubmit = (formData: FormValues) => {
		if (orgId && currentItem.id) {
			mutate({
				unitOfMeasureId: currentItem.id,
			...
			<DialogContent onInteractOutside={handleCloseModal} onEscapeKeyDown={handleCloseModal}>
				<div className="commonDialog">
					<div className="commonDialog__header">
						<DialogTitle asChild>
							<h3 className="commonDialog__title">Edit Unit of Measure</h3>
						</DialogTitle>
					</div>

...
import { useDeleteBrand } from "../api/mutations/useDeleteBrand";
...
export const DeleteDialog: FC<DeleteDialogProps> = ({
	...
	const { mutate, error, isError, isPending, isSuccess } = useDeleteBrand();
	...
	const handleOnSubmit = () => {
		if (currentItem.id && orgId) {
			mutate({ brandId: currentItem.id, organisationId: orgId });
		}
	};
		...
		<DialogRoot open={dialogState} onOpenChange={setDialogState}>
			<DialogContent size="small" onInteractOutside={handleCloseModal} onEscapeKeyDown={handleCloseModal}>
				<div className="infoDialog">
					...
					<DialogTitle asChild>
						<h3 className="infoDialog__title">Delete Brand</h3>
					</DialogTitle>

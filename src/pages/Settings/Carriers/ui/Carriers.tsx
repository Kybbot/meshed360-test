...
import { useGetAllCarriers } from "../api/queries/useGetAllCarriers";
...
const Carriers: FC = () => {
	...
	const [currentItem, setCurrentItem] = useState<CarrierType | null>(null);
	...
	const { data, error, isLoading, isError, isSuccess } = useGetAllCarriers({
	...
	const handleOnEdit = (value: CarrierType) => {
		setCurrentItem(value);
		setEditDiaglog(true);
	};

	const handleOnDeleteBtn = (value: CarrierType) => {
		setCurrentItem(value);
			...
			<div className="main__container">
				<CommonPageWrapper>
					{isLoading ? (
						...
						<>
							<CommonPageHeader>
								<CommonPageTitle>Carriers</CommonPageTitle>
								<CommonPageActions>
									<Search
										id="searchId"
										name="search"
								...
								<div className="customTable">
									<div className="customTable__wrapper">
										<Table>
											...
											<TableTbody>
												{!data.data.carriers.length ? (
													<TableEmpty colSpan={2} />
												) : (
													<>
														{data.data.carriers.map((item) => (
															<TableTr key={item.id}>

...
import { useGetAllSupplierContacts } from "../../../api/queries/useGetAllSupplierContacts";
...
export const Contacts: FC = () => {
	...
	const [currentItems, setCurrentItems] = useState<ContactsType[] | null>(null);

	const { data, error, isLoading, isError, isSuccess } = useGetAllSupplierContacts({
		pageNumber: page,
		pageSize: limit,
		supplierId,
	});

	const { mutate: updateAddress, isPending: isUpdateAddressLoading } = useUpdateSupplierContacts(true);

	const handlePageAndLimit = (page: string, limit: string) => {
		setPageAndLimit({ page, limit });
	};

	const handleChangeCheckbox = (value: ContactsType[], itemIndex: number, checked: boolean) => {
		if (supplierId) {
			...
			updateAddress({ supplierId, body: updatedArr });
		}
	};

	const handleOnNew = (value: ContactsType[]) => {
		setCurrentItems(value);
		setNewDiaglog(true);
	};

	const handleOnEdit = (value: ContactsType[], itemIndex: number) => {
		setCurrentItemIndex(itemIndex);
		setCurrentItems(value);
		setEditDiaglog(true);
	};

	const handleOnDeleteBtn = (value: ContactsType[], itemIndex: number) => {
		setCurrentItemIndex(itemIndex);
		setCurrentItems(value);
			...
			{isLoading ? (
				...
				<>
					<div className="commonPage__actions commonPage__actions--complex">
						<div />
						<Button type="button" onClick={() => handleOnNew(data.data.contacts)} disabled={!supplierId}>

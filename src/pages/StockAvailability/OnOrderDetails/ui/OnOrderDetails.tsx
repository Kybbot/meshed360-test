...
import { useGetOnOrderDetails } from "../api/useGetOnOrderDetails";
...
const OnOrderDetails: FC = () => {
	...
	const { data, error, isLoading, isError, isSuccess } = useGetOnOrderDetails({
		...
		<CommonPage>
			<div className="main__container">
				{isLoading ? (
					...
					<>
						<CommonPageHeader>
							<CommonPageTitle>On Order Details</CommonPageTitle>
							<CommonPageActions>
								<Search
									id="searchOnOrder"
									name="search"
							...
							<div className="customTable">
								<div className="customTable__wrapper">
									<Table>
										<TableThead>
											<TableTr>
												<TableTh>Purchase Order Number</TableTh>
										...
										<TableTbody>
											{data.data.data.length > 0 ? (
												data.data.data.map((item, index) => (
													<TableTr key={`${item.purchaseOrderId}-${index}`}>
														<TableTd>
															<TableLink to={`/purchases/orders/edit/${item.purchaseOrderId}`}>
